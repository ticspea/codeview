(function($, _) { // wrap in anonymous function to not show some helper variables
   ...
   var translations = {
     ...
     en: {
       trash_label: 'Arraste apartir daqui',
       solution_label: 'Construa a tua solução',
       order: function() {
         return "As linhas de código do seu programa estão errados ou na ordem errada. Isto pode ser corrigido movendo, removendo ou substituindo os fragmentos destacados.";},
       lines_missing: function() {
         return "O seu programa possui poucas linhas de código.";},
       lines_too_many: function() {
         return "O seu programa possui muitos fragmentos de código.";},
       no_matching: function(lineNro) {
         return "Com base na sintaxe do algoritmo, o fragmento destacado (" + lineNro + ") não apresenta a devida indentação."; },
       no_matching_open: function(lineNro, block) {
         return "The " + block + " terminou na linha " + lineNro + " nunca iniciou. "; },
       no_matching_close: function(lineNro, block) {
         return "Block " + block + " definifo na linha " + lineNro + " não terminou devidadamente";
       },
       block_close_mismatch: function(closeLine, closeBlock, openLine, inBlock) {
         return "Não é possível terminar o bloco " + closeBlock + " na linha " + closeLine + " quando ainda dentro do bloco " + inBlock + " iniciou na linha " + openLine;
       },
       block_structure: function(lineNro) { return "As linhas destacadas " + lineNro + " pertencem a um bloco ou indentação errada(i.e. indentação)."; },
       unittest_error: function(errormsg) {
         return "<span class='msg'>Erro ao analisar/executar o seu programa</span><br/> <span class='errormsg'>" + errormsg + "</span>";
       },
       unittest_output_assertion: function(expected, actual) {
        return "Resultado esperado: <span class='Resultado esperado'>" + expected + "</span>" +
       ...
       variabletest_assertion: function(varname, expected, actual) {
        return "Expected value of variable " + varname + ": <span class='expected'>" + expected + "</span><br>" +
              "Valor actual: <span class='actual'>" + actual + "</span>";
       }
     }
   };
  ...
  VariableCheckGrader.prototype._python_exec = function(code) {
      ...
      function builtinRead(x) {
        if (Sk.builtinFiles === undefined || Sk.builtinFiles["files"][x] === undefined)
          throw "Ficheiro não encontrado: '" + x + "'";
