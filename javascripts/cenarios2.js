...
const regulo10 = document.getElementById("regulo10");
const cofre3 = document.getElementById("cofre3");
const cofre03 = document.getElementById("cofre03");
...
function moveElement(element, targetX, callback) {
    let currentX = element.getBoundingClientRect().right;
    let targetPosition = targetX;
    ...
    function step() {
        ...
        currentX -= speed3;
        element.style.transform = `translateX(${currentX - targetX}px)`;
    }
    
    interval3 = setInterval(step, 50);
}
...
function startAnimation3() {
    ...
    moveElement(manIdle3, moeda3.getBoundingClientRect().left, () => {
        manIdle3.classList.add("hidden");
        moeda3.classList.add("hidden");
        regulo10.classList.remove("hidden");
        regulo10.style.transform = manIdle3.style.transform;

        moveElement(regulo10, cofre3.getBoundingClientRect().left + 15, () => {
            regulo10.classList.add("hidden");
            manIdle3.classList.remove("hidden");
            manIdle3.style.transform = regulo10.style.transform;
            cofre03.classList.remove("hidden");

            setTimeout(() => {
                cofre03.classList.add("hidden");
                cofre3.classList.remove("hidden");
                isAnimating3 = false;
            }, 3000);
        });
    });
